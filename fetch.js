...
function to_list(data){
	...
	for(var i = 2 ; i < data.length ; i++)
	{
		...
		item.p = temp[7]; 													// 價差百分比
		item.n = parseFloat(temp[8].replace(/,/g,'')); 						// 成交筆數
		list.push(item);
	}
...
function Fetch(stock_no) {
	...
	query.descending('t').first().then(function(latest) {
		...
		// var promises = [];
		// // ugly while loop to add 1 promise per second
		// while(today > latest_date)
		// {
		// 	promises.push(updateMonthly(stock_no, latest_date, today));
		// 	today.setMonth(today.getMonth()-1);
		// }
		// Promise.all(promises)
		// .then( function(ret_arr) {
		// 	var total = 0;
		// 	for(var i = 0 ; i < ret_arr.length ; i++)
		// 	{
		// 		total += ret_arr[i];
		// 	}
		// 	console.log(total + ' records were added.');
		// }, function(e) {
		...
		{
			arrangeTasks(i, stock_no, new Date(latest_date), new Date(today));
			today.setMonth(today.getMonth()-1);
		}
	});
}

function arrangeTasks(i, stock_no, latest_date, today)
{
	setTimeout( function() {
		updateMonthly(stock_no, latest_date, today)
		.then( function(ret) {
			console.log(ret + ' records were added.');
		}, function(e) {
			console.error(e.toString());
		});
	}, 1000*i);
